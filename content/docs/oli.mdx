...
- **Compliance** comes from `getPolicy()` against the TIP-403 precompile — the authoritative policy state every TIP-20 transfer is evaluated against.
...
- `composite_risk` is a weighted combination of measurements — peg deviation, peg-break history, supply headroom, policy type — each of which is itself a direct read.
...
If a value cannot be measured, it is returned as `null` with an explanatory note in the response. Pellet never invents a number to fill a gap.
