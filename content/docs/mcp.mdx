...
Every value the MCP server exposes is an **Open-Ledger Intelligence (OLI)** measurement — read directly from Tempo's public ledger, never aggregated from off-chain oracles. An agent calling Pellet's MCP is practicing OLI on your behalf. See [What is OLI?](/docs/oli).
