...
```bash
curl 'https://pelletfi.com/api/v1/stablecoins/0x20c0.../reserves?as_of=7d'
```

Returns the most recent reserves snapshot per `reserve_type` at or before `as_of`. History is append-only from `2026-04-14`.
