...
import { agents, agentEvents, oliWebhookSubscriptions, oliWebhookDeliveries } from "@/lib/db/schema";
...
type SubscriptionRow = typeof oliWebhookSubscriptions.$inferSelect;
type DeliveryRow = typeof oliWebhookDeliveries.$inferSelect;
...
export async function dispatchToWebhooks(eventId: number): Promise<void> {
  ...
  const inserted = await db
    ...
    .onConflictDoNothing({
      target: [oliWebhookDeliveries.subscriptionId, oliWebhookDeliveries.eventId],
    })
    .returning({ id: oliWebhookDeliveries.id });
...
async function loadDelivery(id: string): Promise<DeliveryRow | null> {
  const rows = await db
    ...
    .where(eq(oliWebhookDeliveries.id, id))
    .limit(1);
  return rows[0] ?? null;
}
...
async function loadSubscription(id: string): Promise<SubscriptionRow | null> {
  const rows = await db
    ...
    .where(eq(oliWebhookSubscriptions.id, id))
    .limit(1);
  return rows[0] ?? null;
}
...
export async function attemptDelivery(deliveryId: string): Promise<void> {
  ...
  if (!subscription) {
    ...
    await db
      ...
      .where(eq(oliWebhookDeliveries.id, deliveryId));
    return;
  }
  ...
  if (outcome.kind === "success") {
    await db.transaction(async (tx) => {
      ...
      await tx
        ...
        .where(eq(oliWebhookSubscriptions.id, subscription.id));
    });
    return;
  }
...
export async function processRetryQueue(limit = 100): Promise<{ attempted: number }> {
  const due = await db
    .select({ id: oliWebhookDeliveries.id })
    .from(oliWebhookDeliveries)
    .where(
      and(
        eq(oliWebhookDeliveries.status, "retry"),
        sql`${oliWebhookDeliveries.nextRetryAt} <= now()`,
      ),
    )
    .orderBy(oliWebhookDeliveries.nextRetryAt)
    .limit(limit);
