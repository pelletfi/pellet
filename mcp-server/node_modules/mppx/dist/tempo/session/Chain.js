import { decodeFunctionData, encodeFunctionData, getAbiItem, toFunctionSelector, } from 'viem';
import { call, prepareTransactionRequest, readContract, sendRawTransaction, sendRawTransactionSync, signTransaction, writeContract, } from 'viem/actions';
 ...
 */
async function sendFeePayerTx(client, account, feePayer, to, data, label) {
...
export async function broadcastOpenTransaction(parameters) {
    const { client, serializedTransaction, escrowContract, channelId, recipient, currency, feePayer, waitForConfirmation = true, } = parameters;
    ...
    if (feePayer) {
        for (const call of calls) {
            ...
            const isEscrowOpen = TempoAddress.isEqual(call.to, escrowContract) && selector === escrowOpenSelector;
            const isTokenApprove = TempoAddress.isEqual(call.to, currency) && selector === erc20ApproveSelector;
    ...
    const resolvedFeeToken = transaction.feeToken ?? defaults.currency[client.chain?.id];
...
export async function broadcastTopUpTransaction(parameters) {
    const { client, serializedTransaction, escrowContract, channelId, currency, declaredDeposit, previousDeposit, feePayer, } = parameters;
    if (feePayer && !isTempoTransaction(serializedTransaction))
        throw new BadRequestError({
            reason: 'Only Tempo (0x76/0x78) transactions are supported',
        });
    ...
    if (feePayer) {
        for (const call of calls) {
            ...
            const selector = call.data.slice(0, 10);
            const isEscrowTopUp = TempoAddress.isEqual(call.to, escrowContract) && selector === escrowTopUpSelector;
            const isTokenApprove = TempoAddress.isEqual(call.to, currency) && selector === erc20ApproveSelector;
        ...
        if (feePayer) {
            return signTransaction(client, {
                ...
                feeToken: transaction.feeToken ??
                    defaults.currency[client.chain?.id],
            });
        }
        return serializedTransaction;
    })();
