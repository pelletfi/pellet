    ...
    const self = {
        ...
        async open(options) {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Open request failed with status ${response.status}${body ? `: ${body}` : ''}${wwwAuth ? ` [WWW-Authenticate: ${wwwAuth}]` : ''}`);
            }
        },
        fetch: doFetch,
        ...
        async ws(input, init) {
            ...
            const challenge = Challenge.fromResponseList(probe).find((item) => item.method === method.name && item.intent === method.intent);
            if (!challenge) {
        ...
        async close() {
            ...
            if (!channel?.opened || !closeChallenge || !closeChannelId)
                return undefined;
            ...
            if (lastUrl) {
                ...
                const receiptHeader = response.headers.get('Payment-Receipt');
                if (receiptHeader)
                    receipt = deserializeSessionReceipt(receiptHeader);
            }
            return receipt;
        },
    };
