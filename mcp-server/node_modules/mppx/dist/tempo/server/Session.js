...
export function session(p) {
    const parameters = p;
    const { amount, channelStateTtl = 5_000, currency = defaults.resolveCurrency(parameters), decimals = defaults.decimals, store: rawStore = Store.memory(), suggestedDeposit, unitType, waitForConfirmation = true, } = parameters;
    ...
    return Method.toServer(Methods.session, {
        ...
        async verify({ credential, request }) {
            ...
            switch (payload.action) {
                case 'open':
                    sessionReceipt = await handleOpen(store, client, challenge, payload, methodDetails, resolvedFeePayer, waitForConfirmation);
                    lastOnChainVerified.set(payload.channelId, Date.now());
                    break;
                case 'topUp':
                    sessionReceipt = await handleTopUp(store, client, challenge, payload, methodDetails, resolvedFeePayer);
                    lastOnChainVerified.set(payload.channelId, Date.now());
                    break;
                case 'voucher':
...
async function verifyAndAcceptVoucher(parameters) {
    const { store, minVoucherDelta, challenge, channel, channelId, voucher, onChain, methodDetails } = parameters;
    if (onChain.finalized) {
        throw new ChannelClosedError({ reason: 'channel is finalized on-chain' });
    }
    if (onChain.closeRequestedAt !== 0n) {
    ...
    const isValid = await verifyVoucher(methodDetails.escrowContract, methodDetails.chainId, voucher, channel.authorizedSigner);
    if (!isValid) {
 ...
 */
async function handleOpen(store, client, challenge, payload, methodDetails, feePayer, waitForConfirmation) {
    const voucher = parseVoucherFromPayload(payload.channelId, payload.cumulativeAmount, payload.signature);
 ...
 */
async function handleTopUp(store, client, challenge, payload, methodDetails, feePayer) {
...
async function handleVoucher(store, client, minVoucherDelta, challenge, payload, methodDetails, channelStateTtl, lastOnChainVerified) {
    ...
    const onChain = await (async () => {
        if (isStale) {
            const onChainChannel = await getOnChainChannel(client, methodDetails.escrowContract, payload.channelId);
            lastOnChainVerified.set(payload.channelId, Date.now());
            // Persist closeRequestedAt so the cached path detects force-close
            // between re-queries.
            if (onChainChannel.closeRequestedAt !== 0n) {
                await store.updateChannel(payload.channelId, (current) => current ? { ...current, closeRequestedAt: onChainChannel.closeRequestedAt } : current);
            }
