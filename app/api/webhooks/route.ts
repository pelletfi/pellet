...
import { validateCallbackUrl, validateFilter } from "@/lib/wallet/webhooks/validation";
import { sendVerifyPing } from "@/lib/wallet/webhooks/dispatcher";
...
export async function GET(req: Request) {
  ...
  const rows = await db
    ...
    .where(
      and(
        eq(webhookSubscriptions.ownerUserId, owner.userId),
        ne(webhookSubscriptions.status, "deleted"),
      ),
    )
